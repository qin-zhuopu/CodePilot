...
| docker-web-deployment.md | **Docker Web 部署**（`release/docker` 分支）：纯 Node.js 容器运行 Web 服务（无 Electron）、多阶段构建、`release-docker.sh <tag>` 参数化发布、数据持久化 volume、功能降级清单、4 个基础镜像坑（uid 1001 / ENTRYPOINT zsh / nvm PATH / NODE_ENV 污染 build）+ 代理污染健康检查 |
