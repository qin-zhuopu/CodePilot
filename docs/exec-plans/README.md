...
| [active/model-capability-reasoning-refresh.md](active/model-capability-reasoning-refresh.md) | **模型目录与推理强度统一适配**：GLM-5.2、Kimi for Coding、GPT-5.6、Claude Sonnet 5 / Opus 5、DeepSeek V4 Flash 0731 / V4 Pro 0813、Grok 4.6，以及 ClinePass / OpenCode Go 聚合边界；按模型、SDK 与网关真实 wire capability 驱动强度选择，不显示伪档位 | ✅ v0.67.1 shipped：GLM Models 热修四个 P2 条件及 Tests/build/isolated UI smoke 完成，CI 全绿，stable Release 12 assets uploaded；修复后 Claude 复审未重跑。Grok 只展示 SDK 可承载的 L/M/H；已知 telemetry flake 见 tech-debt #86；真实 xAI/GLM 凭据 smoke 待完成 |
